...
def schedule_offsetfn(t):
    ...
    # Trigger a crash
    if not market_state['crash_active'] and market_state['crash_start_time'] is None and math.floor(t) == 300:
...
rangeS = (100, 200, schedule_offsetfn)
rangeD = rangeS
...
for n in num_traders:
    for trader_type in trader_types:
        trial_id = f'test_{trader_type}'
        buyers_spec = [(trader_type, 100)]  # All buyers are of the same type
        ...
        # Plot trade prices
        axes[0].scatter(trades_df_reset['Time'], trades_df_reset['Price'], marker='x', label=f'{trader_type}',
                        color='black')
        axes[0].set_ylabel("Trade Price")
        axes[0].set_title(f"Trade Prices Over Time ({trader_type})")
        axes[0].legend()
        axes[0].grid()
        ...
        axes[1].scatter(entropy_plot_df['Time'], entropy_plot_df['Entropy'], marker='x',
                        label=f'Entropy ({trader_type})', color='blue')
        axes[1].set_xlabel("Time")
        axes[1].set_ylabel("Entropy")
        axes[1].set_title(f"Price Entropy Over Time ({trader_type})")
        axes[1].legend()
        axes[1].grid()
        ...
        pre_shock = trades_df_reset[(trades_df_reset['Time_dt'] >= shock_time - pd.Timedelta(seconds=30)) &
                                    (trades_df_reset['Time_dt'] < shock_time)]
        post_shock = trades_df_reset[(trades_df_reset['Time_dt'] >= shock_time) &
                                     (trades_df_reset['Time_dt'] < shock_time + pd.Timedelta(seconds=30))]
