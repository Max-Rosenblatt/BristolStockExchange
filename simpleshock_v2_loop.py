...
def run_simulation(args):
    # Unpack arguments:
    sim_num, buyer_type, seller_type, n_traders, range1, range2 = args
    ...
    # Configure market session schedules for supply/demand:
    schedule = [
...
def run_parallel_simulations(n_simulations, n_traders, trader_types, output_filename, range1, range2):
    ...
    for buyer_type in trader_types:
        for seller_type in trader_types:
            for sim in range(1, n_simulations + 1):
                args_list.append((sim, buyer_type, seller_type, n_traders, range1, range2))
...
if __name__ == '__main__':
    n_simulations = 100  # 100 simulations per buyer-seller combination
    n_traders = 50

    # Define the four trader types for buyers and sellers
    trader_types = ['ZIP', 'GVWY', 'SHVR', 'ZIC']

    # Price ranges for pre and post shock period
    range1 = (100, 150)
    range2 = (80, 130)

    output_filename = f'simpleshock_entropy_results_{n_simulations}sims_{n_traders}trad_allCombinations.csv'
    ...
    if os.path.exists(output_filename):
        ...
        if response.lower() != 'y':
            ...
            run_parallel_simulations(n_simulations, n_traders, trader_types, output_filename, range1, range2)
    else:
        run_parallel_simulations(n_simulations, n_traders, trader_types, output_filename, range1, range2)
