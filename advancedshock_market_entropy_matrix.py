...
fig, axes = plt.subplots(len(buyer_types), len(seller_types), figsize=(16, 14), sharex=True, sharey=True)
...
for row_idx, buyer in enumerate(buyer_types):
    for col_idx, seller in enumerate(seller_types):
        ...
        if col_idx == 0:
            ax.set_ylabel(f'{buyer}', rotation=0, labelpad=10, va='center', ha='right')
        ...
        ax.set_xticks([])
        if col_idx == 0:
            ax.set_ylabel(f'{buyer}', rotation=0, labelpad=50, va='center', ha='right')
...
# Add shared labels
fig.text(0.02, 0.5, 'Buyer Type', va='center', rotation='vertical')
