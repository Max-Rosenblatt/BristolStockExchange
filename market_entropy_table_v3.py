...
def main():
    ...
    trader_counts = np.linspace(10,100,10,dtype=int)
    n_simulations = 5
    price_range = (100, 150)
    output_filename = 'entropy_by_trader_types_variedN.csv'
    ...
    if os.path.exists(output_filename):
        ...
        existing_df = pd.DataFrame(columns=[
            'simulation_number', 'n_traders', 'buyer_type', 'seller_type',
            'average_entropy', 'entropy_stderr', 'num_trades'
        ])

    all_combinations = {(sim, n, bt, st)
                        for n in trader_counts
    ...
    if missing_combinations:
        ...
        args_list = [
            (sim, n, bt, st, output_filename, price_range)
            for sim, n, bt, st in missing_combinations
        ]
        ...
        new_df['n_traders'] = [arg[1] for arg in args_list]  # Add n_traders to the results
        combined_df = pd.concat([existing_df, new_df], ignore_index=True)
        combined_df.sort_values(by=['n_traders', 'buyer_type', 'seller_type', 'simulation_number'], inplace=True)
    ...
    # Plot grouped by n_traders
    for n in trader_counts:
        print(f"\nPlotting results for {n} traders...")
        plot_results(df[df['n_traders'] == n], trader_types)

    # Summary statistics across all trader counts
    summary_df = df.groupby(['n_traders', 'buyer_type', 'seller_type']).agg({
    ...
    summary_df.to_csv('entropy_summary_statistics_by_n_traders.csv', index=False)
